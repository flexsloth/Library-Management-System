...
def loginlibr():
    global window
    ret=Label(win,text='User Panel ',font='Helvetica 30 bold')
    ret.place(x=55,y=30)
    connectdb()
...
def addbook():
    ...
    b1=Button(win, height=2,width=21,text=' CLOSE ',command=closebook)
    sub.place(x=70,y=50)
    tit.place(x=70,y=90)
...
def viewbook():
    ...
    treeview=Treeview(win,columns=("Type","Title","Author","Serial No"),show='headings')
...
def issuedbook():
    ...
    if len(details)!=0:
        ...
        win.geometry("800x300+270+180")
        win.resizable(False,False)    
        treeview=Treeview(win,columns=("Student ID","Title","Issue Date","Expiry Date"),show='headings')
...
def deletebook():
    ...
    b2=Button(win, height=2,width=17,text=' CLOSE ',command=closebook)
...
def admin():
    ...
    b4=Button(win, height=2,width=25,text=' Add Book ',command=addbook)
    b5=Button(win, height=2,width=25,text=' Delete Book ',command=deletebook)
    b6=Button(win, height=2,width=25,text=' View Issued Books ',command=issuedbook)
    b7=Button(win, height=2,width=25,text=' View Book ',command=viewbook)
...
def addusers():
    connectdb()
    q='INSERT INTO Login VALUE("%s","%i","%s","%d")'
    global con,cur
...
def connectdb():
    ...
    con=p.connect(host="localhost",user="root",passwd="boombaam@1234")
    cur=con.cursor()
    cur.execute('CREATE DATABASE IF NOT EXISTS Library1')
    cur.execute('USE Library1')
    global enter
    if enter==1:
        l='CREATE TABLE IF NOT EXISTS Login(name varchar(20),userid varchar(10),branch varchar(20),mobile int(30))'
        b='CREATE TABLE IF NOT EXISTS Book(type varchar(20),title varchar(20),author varchar(20),serial int(5))'
...
def home():
    try:
        ...
        wel=Label(window,text='LIBRARY MANAGEMENT',font='Helvetica 22 bold')
        lib=Label(window,text='SYSTEM',font='Helvetica 22 bold')
        usid=Label(window,text='USER ID')
        paswrd=Label(window,text='PASSWORD')
        e1=Entry(window,width=22)
        e2=Entry(window,width=22)
        b1=Button(window,text=' LOGIN AS USER' ,height=2,width=20,command=loginlibr)
        b2=Button(window,text=' LOGIN AS ADMIN ', height=2,width=20,command=loginadmin)
        b3=Button(window,text=' EXIT ', height=2,width=20,command=exit)
        wel.place(x=30,y=0)
        lib.place(x=110,y=40)
        usid.place(x=70,y=100)
        paswrd.place(x=70,y=140)
        e1.place(x=180,y=100)
        e2.place(x=180,y=140)
        b2.place(x=55,y=180)
        b1.place(x=205,y=180)
        b3.place(x=125,y=230)
        window.mainloop()
    except Exception:
        window.destroy()
